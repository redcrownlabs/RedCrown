...
impl Session {
    ...
    pub fn new_with_opts(
        ...
        async move {
            let peer_id = opts.peer_id.unwrap_or_else(generate_tracker_safe_peer_id);
            ...
            let reqwest_client = {
                ...
                builder
                    .user_agent(crate::client_name_and_version())
                    .build()
    ...
    fn make_peer_rx(
        ...
        let (tracker_announce_port, dht_announce_port) =
            peer_discovery_announce_ports(self.tcp_listen_port, announce);
        let dht_rx = if is_private {
            None
        } else {
            self.dht
                .as_ref()
                .map(|dht| dht.get_peers(info_hash, dht_announce_port))
        };
...
/// Separates tracker protocol validity from swarm-presence announcements.
///
/// HTTP trackers require a nonzero listening port even while a client only
/// resolves magnet metadata. DHT receives the port only when the torrent is
/// active, preserving the existing list-only presence behavior.
fn peer_discovery_announce_ports(
    tcp_listen_port: Option<u16>,
    announce_presence: bool,
) -> (Option<u16>, Option<u16>) {
    let dht_announce_port = announce_presence.then_some(tcp_listen_port).flatten();
    (tcp_listen_port, dht_announce_port)
}

/// Generates an Azureus-style peer ID accepted by strict HTTP trackers.
///
/// The Azureus suffix is conventionally printable. Keeping it alphanumeric
/// also prevents tracker WAFs from rejecting percent-encoded arbitrary bytes.
fn generate_tracker_safe_peer_id() -> Id20 {
    let mut peer_id = generate_azereus_style(*b"rQ", crate_version!());
    let mut rng = rand::rng();
    for byte in &mut peer_id.0[8..] {
...
mod tests {
    ...
    #[test]
    fn generated_peer_id_uses_printable_azureus_suffix() {
        let peer_id = generate_tracker_safe_peer_id();

        assert_eq!(&peer_id.0[..3], b"-rQ");
        assert_eq!(peer_id.0[7], b'-');
        assert!(peer_id.0[8..].iter().all(u8::is_ascii_alphanumeric));
    }

    #[test]
    fn metadata_only_discovery_keeps_tracker_port_without_announcing_to_dht() {
        assert_eq!(
            peer_discovery_announce_ports(Some(49_152), false),
            (Some(49_152), None)
        );
    }

    #[test]
    fn active_discovery_announces_bound_port_everywhere() {
        assert_eq!(
