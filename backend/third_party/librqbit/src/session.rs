...
use crate::{
    ...
    bitv_factory::{BitVFactory, FolderBitVFactory, NonPersistentBitVFactory},
    create_torrent,
...
pub struct SessionOptions {
    ...
    /// Stores fast-resume bitfields without persisting torrent membership.
    ///
    /// Each bitfield is placed below `<root>/<info-hash>/`, allowing a caller's
    /// cache lifecycle to remove media bytes and verification state atomically.
    /// This cannot be combined with [`Self::persistence`].
    pub fastresume_root: Option<PathBuf>,
    ...
    pub fn new_with_opts(
        ...
        async move {
            ...
            async fn persistence_factory(
                ...
                if opts.persistence.is_some() && opts.fastresume_root.is_some() {
                    bail!("persistence and fastresume_root cannot be configured together");
                }
                if !opts.fastresume && opts.fastresume_root.is_some() {
                    bail!("fastresume_root requires fastresume=true");
                }
                ...
                match &opts.persistence {
                    ...
                    None if opts.fastresume => {
                        let root = opts
                            .fastresume_root
                            .clone()
                            .context("fastresume=true requires persistence or fastresume_root")?;
                        Ok((
                            None,
                            Arc::new(FolderBitVFactory::new(root, spawner)) as Arc<dyn BitVFactory>,
                        ))
                    }
            ...
            let add_res = match add {
                ...
                other => {
                    let torrent = match other {
                        ...
                        AddTorrent::Url(url) => {
                            bail!("unsupported URL {url:?}. Supporting magnet:, http:, and https")
                        }
        ...
        .await
        {
            ReadMetainfoResult::Found {
                ...
                Ok(ResolveMagnetResult {
                    ...
                    peer_rx: rx,
                    seen_peers: {
                        let mut prioritized = Vec::with_capacity(seen.len());
                        prioritized.push(source);
                        prioritized.extend(seen.into_iter().filter(|peer| *peer != source));
                        for peer in &prioritized {
                            trace!(?peer, "seen")
                        }
                        prioritized
                    },
